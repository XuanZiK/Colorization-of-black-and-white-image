...
Formatted source-code listing (appendix to the thesis).
...
All source code and model checkpoints used in the thesis.
...
Runs both trained models on the validation split and saves rendered RGB images to disk.

benchmark_sweep.py

Sweeps the benchmark over multiple sample counts and plots average time per image.
...
Visual validation of one random or indexed sample, including annealed-mean decoding.
...
Checkpoint: best variant reported in the thesis.
...
Checkpoint: low-LR variant (default used by the benchmark).

(only-regression)main-model.pt

Regression-only variant (no GAN).
...
The 313 ab-bin centres used by the classification pipeline.
...
Disk
Approximately 2.5 GB for the provided model checkpoints.
Approximately 2.6 GB for the gray_scale.npy + 3× ab*.npy dataset arrays (see Section 4 — not included in this repository/archive).
...
torch==2.7.1
torchvision==0.22.1
No IDE is required. Any terminal with Python installed is sufficient.

(VS Code was used during development; a .vscode/ folder is included but is purely optional.)
...
pts_in_hull.npy does not need to be supplied manually.

It contains the 313 ab-bin centres used by the classification pipeline.

If it is missing, load_lab_npy_data.py will generate it automatically by k-means on the first run and cache it to:
...
This renders 500 validation images with each of the two reported models and prints timings.
...
The rendered figure is saved as:

Text
validation_result_idx_<N>.png
...
Training hyper-parameters live in the TrainConfig dataclass at the top of each training script.

Defaults match the runs reported in the thesis:
