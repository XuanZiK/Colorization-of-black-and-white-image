...
VISUALIZE_SELECT_MODE = "random"   # "random" or "by_index"
SELECTED_SAMPLE_IDX = 0
RANDOM_SEED = 42
BATCH_SIZE_FOR_VAL = 4


ANNEALED_TEMPERATURE = 0.38

# 是否启用 annealed mean；False 时退化为普通 softmax expectation
USE_ANNEALED_MEAN = True
# ===========================================
...
def logits_to_ab_annealed_mean(
    logits: torch.Tensor,
    color_bins_lab: torch.Tensor,
    temperature: float = 0.38,
    use_annealed_mean: bool = True,
) -> torch.Tensor:

    if logits.ndim != 4:
        raise ValueError(f"logits 应为 4D 张量 [B,313,H,W]，实际为 {tuple(logits.shape)}")

    if color_bins_lab.ndim != 2 or color_bins_lab.shape[1] != 2:
        raise ValueError(f"color_bins_lab 应为 [313,2]，实际为 {tuple(color_bins_lab.shape)}")

    if logits.shape[1] != color_bins_lab.shape[0]:
        raise ValueError(
            f"logits 通道数 {logits.shape[1]} 与 color bin 数 {color_bins_lab.shape[0]} 不匹配"
        )

    if use_annealed_mean:
        probs = torch.softmax(logits / temperature, dim=1)
    else:
        probs = torch.softmax(logits, dim=1)

    # [313,2] -> [1,313,2,1,1]
    bins = color_bins_lab.view(1, color_bins_lab.shape[0], 2, 1, 1)

    # probs: [B,313,H,W] -> [B,313,1,H,W]
    probs = probs.unsqueeze(2)

    # 加权期望: sum_q p_q * c_q
    ab_lab = (probs * bins).sum(dim=1)   # [B,2,H,W]

    ab_norm = torch.clamp(ab_lab / 128.0, -1.0, 1.0)
    return ab_norm


def logits_to_ab_argmax(logits: torch.Tensor, color_bins_lab: torch.Tensor) -> torch.Tensor:
...
def main():
    ...
    try:
        ...
        if out_channels == 313:
            ...
            print(f"🎨 已加载 color bins: {tuple(color_bins_lab.shape)}")
            if USE_ANNEALED_MEAN:
                print(f"🔥 分类解码方式: annealed mean (T={ANNEALED_TEMPERATURE})")
            else:
                print("📊 分类解码方式: standard softmax expectation (no temperature scaling)")
    ...
    with torch.no_grad():
        ...
        elif output.shape[1] == 313:
            ab_pred = logits_to_ab_annealed_mean(
                output,
                color_bins_lab,
                temperature=ANNEALED_TEMPERATURE,
                use_annealed_mean=USE_ANNEALED_MEAN,
            )
            if USE_ANNEALED_MEAN:
                pred_title = f"Predicted Color (313ch, Annealed Mean, T={ANNEALED_TEMPERATURE})"
            else:
                pred_title = "Predicted Color (313ch, Softmax Expectation)"
        else:
            raise RuntimeError(f"不支持的输出通道数: {output.shape[1]}")
